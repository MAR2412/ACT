...
>
    ...
    <div class="relative">
        <input
            ...
            type="text"
            id="{{ $id }}"
            {{ $attributes->merge(['class' => 'w-full rounded-md border-stone-300 dark:border-stone-700 dark:bg-stone-900 dark:text-stone-300 focus:border-green-500 dark:focus:border-stone-600 focus:ring-green-500 dark:focus:ring-stone-600 shadow-sm py-2 pl-3 pr-10']) }}
        ...
        <div class="absolute inset-y-0 right-0 flex items-center px-2">
            <button 
                type="button" 
                class="h-full px-1 text-stone-400 focus:outline-none"
                @click="showPicker = !showPicker"
            >
    ...
    <div
        ...
        class="w-64 bg-white dark:bg-stone-800 border border-stone-300 dark:border-stone-700 rounded-lg shadow-lg overflow-hidden"
    >
        <!-- Header con navegación por décadas -->
        <div class="bg-stone-50 dark:bg-stone-700 px-4 py-2 flex justify-between items-center">
            <button 
                type="button"
                @click="previousDecade()" 
                @click.stop
                class="text-stone-600 dark:text-stone-300 hover:text-stone-800 dark:hover:text-stone-100"
            ...
            <span x-text="currentDecade + ' - ' + (currentDecade + 9)" class="font-semibold text-stone-800 dark:text-stone-200"></span>
            <button 
                type="button"
                @click="nextDecade()"
                @click.stop
        ...
        <!-- Footer con acciones -->
        <div class="bg-stone-50 dark:bg-stone-700 px-4 py-2 flex justify-between">
            <button 
                type="button"
                @click="selectYear(currentYear)"
                @click.stop 
                class="text-sm text-green-600 dark:text-stone-400 hover:text-green-800 dark:hover:text-stone-300 font-medium"
            >
                Hoy
            </button>
            <button 
                ...
                @click.stop 
                class="text-sm text-stone-600 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-200 font-medium"
            >
                Cancelar
            </button>
        </div>
