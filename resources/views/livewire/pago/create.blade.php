                ...
                <div class="border border-stone-200 dark:border-stone-700 rounded-md p-4 {{ $index > 0 ? 'mt-4' : '' }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <div class="flex space-x-2">
                                <x-input 
                                    ...
                                    type="number" 
                                    class="flex-grow" 
                                    placeholder="Ingrese ID de matrícula"
                                />
                                <x-button 
                                    type="button" 
                            ...
                            @error("pagosForms.{$index}.matricula_id") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <x-label value="Tipo de Pago *" />
                            <select 
                                wire:model.defer="pagosForms.{{ $index }}.tipo" 
                        ...
                        <div>
                            ...
                            <select 
                                wire:model.defer="pagosForms.{{ $index }}.metodo_pago" 
                                class="w-full border-stone-300 dark:border-stone-600 dark:bg-stone-700 dark:text-white rounded-md shadow-sm focus:border-emerald-500 focus:ring-emerald-500"
                            >
                        ...
                        <div>
                            ...
                            <x-input 
                                ...
                                class="w-full"
                            />
                            @error("pagosForms.{$index}.mes_pagado") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <x-label value="Estado *" />
                            <select 
                                wire:model.defer="pagosForms.{{ $index }}.estado" 
                                class="w-full border-stone-300 dark:border-stone-600 dark:bg-stone-700 dark:text-white rounded-md shadow-sm focus:border-emerald-500 focus:ring-emerald-500"
                            >
                        ...
                        <div>
                            ...
                            <x-input 
                                wire:model.defer="pagosForms.{{ $index }}.numero_transaccion" 
                                class="w-full"
                        ...
                        <div>
                            ...
                            <x-input 
                                wire:model.defer="pagosForms.{{ $index }}.referencia_bancaria" 
                                class="w-full"
                    ...
                    <div class="mt-4">
                        ...
                        <textarea 
                            wire:model.defer="pagosForms.{{ $index }}.observaciones" 
                            rows="2"
                    ...
                    @if(!$isEditing && !empty($form['matricula_id']) && !empty($form['matricula_info']))
                        <div class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md">
                            <div class="flex justify-between items-start">
                                <div class="flex-grow">
                                    ...
                                    @if($form['matricula_info']->examen_suficiencia)
                                        <div class="mt-1">
                                    ...
                                    @endif
                                    @if($form['matricula_info']->descuento_aplicado)
                                        <div class="mt-1">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300">
                                                ...
                                                Descuento {{ $form['matricula_info']->porcentaje_descuento }}%
                                            </span>
                                        </div>
                                    @endif
                                    <div class="grid grid-cols-2 gap-2 text-sm text-blue-600 dark:text-blue-400 mt-2">
                                        <div>
                                            <span class="font-medium">Módulo:</span> {{ $form['matricula_info']->modulo->nombre }}
                                        </div>
                                        <div>
                                            <span class="font-medium">Duración:</span> {{ $form['matricula_info']->modulo->duracion_meses }} meses
                                        </div>
                                        <div>
                                            <span class="font-medium">Precio mensual:</span> L. {{ number_format($form['matricula_info']->modulo->precio_mensual, 2) }}
                                        </div>
                                        <div>
                                            <span class="font-medium">Total módulo:</span> L. {{ number_format($form['matricula_info']->precio_total_modulo, 2) }}
                                            @if($form['matricula_info']->descuento_aplicado)
                                                <div class="text-xs text-stone-500 dark:text-stone-400 line-through">
                                                    Original: L. {{ number_format($form['matricula_info']->precio_original, 2) }}
                                                </div>
                                            @endif
                                        </div>
                                        <div class="col-span-2">
                                            <span class="font-medium">Meses:</span> 
                                            <span class="text-emerald-600 dark:text-emerald-400">{{ $form['matricula_info']->meses_pagados }} pagados</span> | 
                                            <span class="text-red-600 dark:text-red-400">{{ $form['matricula_info']->meses_pendientes }} pendientes</span>
                                        </div>
                                        <div class="col-span-2">
                                        ...
                                        @if($form['matricula_info']->fecha_proximo_pago)
                                        <div class="col-span-2">
                                            <span class="font-medium">Próximo pago:</span> 
                                            {{ \Carbon\Carbon::parse($form['matricula_info']->fecha_proximo_pago)->format('d/m/Y') }}
                                        </div>
                                        @endif
                                    </div>
                                </div>
                        ...
                        @elseif(!$isEditing && !empty($form['matricula_id']) && empty($form['matricula_info']))
                        <div class="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md text-center">
                            <p class="text-yellow-600 dark:text-yellow-400 text-sm">
                                Matrícula ID: {{ $form['matricula_id'] }} cargada
                            </p>
                            <x-button 
                                type="button" 
                                wire:click="buscarMatricula({{ $index }})"
