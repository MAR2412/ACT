<div>
    <div class="mx-auto rounded-lg mt-8 sm:mt-6 lg:mt-4 mb-6">
        <div class="bg-white dark:bg-white/5 overflow-hidden shadow sm:rounded-lg p-4 sm:p-6">
            ...
            <div class="mb-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                    <h2 class="text-xl font-semibold text-stone-800 dark:text-stone-200">
                        {{ __('Administración de Modalidades') }}
                    </h2>

                    <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                        <div class="relative w-full sm:w-auto">
                            <x-input wire:model.live="search" type="text" placeholder="Buscar por nombre..."
                        ...
                        @can('secciones.modalidades.crear')
            ...
            <x-table
                ...
                empty-message="No se encontraron modalidades"
                class="mt-6"
            >
                <x-slot name="desktop">
                    @forelse($modalidades as $modalidad)
                        <tr class="hover:bg-stone-50 dark:hover:bg-stone-700 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">{{ $modalidad->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">{{ $modalidad->nombre }}</td>
                            <td class="px-6 py-4 text-stone-900 dark:text-stone-300">
                                {{ Str::limit($modalidad->descripcion, 50) ?? 'Sin descripción' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <button wire:click="toggleEstado({{ $modalidad->id }})"
                                        class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 {{ $modalidad->estado ? 'bg-emerald-600' : 'bg-gray-200 dark:bg-gray-700' }}">
                                        <span class="sr-only">Cambiar estado</span>
                                        <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {{ $modalidad->estado ? 'translate-x-6' : 'translate-x-1' }}"></span>
                                    </button>
                                    <span class="ml-2 text-sm text-stone-600 dark:text-stone-400">
                                        {{ $modalidad->estado ? 'Activo' : 'Inactivo' }}
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    @if($modalidad->trashed())
                                        @can('secciones.modalidades.restaurar')
                                        ...
                                        @endcan
                                    @else
                                        @can('secciones.modalidades.editar')
                                        ...
                                        @endcan
                                        @can('secciones.modalidades.eliminar')
                ...
                <x-slot name="mobile">
                    @forelse($modalidades as $modalidad)
                        <div class="bg-white dark:bg-stone-800 p-4 rounded-lg shadow-sm border border-stone-200 dark:border-stone-700 mb-3">
                            ...
                            <h3 class="font-semibold text-stone-900 dark:text-stone-200 text-lg mb-1">{{ $modalidad->nombre }}</h3>
                            <p class="text-sm text-stone-600 dark:text-stone-400 mb-2">
                                {{ Str::limit($modalidad->descripcion, 80) ?? 'Sin descripción' }}
                            </p>
    ...
    @include('livewire.modalidad.delete-confirmation')
    @include('livewire.modalidad.error-modal')
</div>