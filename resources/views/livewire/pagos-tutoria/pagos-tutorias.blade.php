            ...
            <div class="mb-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                    <h2 class="text-xl font-semibold text-stone-800 dark:text-stone-200">
                        {{ __('Administración de Pagos de Tutorías') }}
                    </h2>

                    <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                        <div class="relative w-full sm:w-auto">
                            <x-input wire:model.live="search" type="text" placeholder="Buscar estudiantes, tutorías, transacciones..."
                        ...
                        @can('pagos-modulos.pagos-tutorias.crear')
                ...
                <x-slot name="desktop">
                    @forelse($pagos as $pago)
                        <tr class="hover:bg-stone-50 dark:hover:bg-stone-700 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                <div class="font-medium">{{ $pago->matriculaTutoria->estudiante->nombre }} {{ $pago->matriculaTutoria->estudiante->apellido }}</div>
                                <div class="text-sm text-stone-500 dark:text-stone-400">
                                    {{ $pago->matriculaTutoria->tutoria->nombre }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                <div class="text-sm">
                                    <div class="font-medium">{{ ucfirst(str_replace('_', ' ', $pago->tipo)) }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                <div class="text-sm">
                                    <div class="font-medium">L. {{ number_format($pago->monto, 2) }}</div>
                                    <div class="text-stone-500 dark:text-stone-400">
                                        Pagado: L. {{ number_format($pago->monto_pagado, 2) }}
                                        @if($pago->cambio > 0)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    @if($pago->trashed())
                                        @can('pagos-modulos.pagos-tutorias.restaurar')
                                        ...
                                        @endcan
                                    @else
                                        @can('pagos-modulos.pagos-tutorias.editar')
                                        ...
                                        @endcan
                                        @can('pagos-modulos.pagos-tutorias.eliminar')
                                        ...
                                        @endcan
                                        <button wire:click="downloadPagoTutoriaPdf({{ $pago->id }})" class="text-green-600 hover:text-green-900">
                    ...
                    @forelse($pagos as $pago)
                        <div class="bg-white dark:bg-stone-800 p-4 rounded-lg shadow-sm border border-stone-200 dark:border-stone-700 mb-3">
                            ...
                            <h3 class="font-semibold text-stone-900 dark:text-stone-200 text-lg mb-1">
                                {{ $pago->matriculaTutoria->estudiante->nombre }} {{ $pago->matriculaTutoria->estudiante->apellido }}
                            </h3>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="text-sm text-stone-600 dark:text-stone-400">
                                    <div>{{ ucfirst(str_replace('_', ' ', $pago->tipo)) }} - {{ ucfirst($pago->metodo_pago) }}</div>
                                    @if($pago->numero_transaccion)
                            ...
                            <div class="mb-3">
                                ...
                                <div class="text-sm text-stone-600 dark:text-stone-400">
                                    <div>Monto: L. {{ number_format($pago->monto, 2) }}</div>
                                    <div>Pagado: L. {{ number_format($pago->monto_pagado, 2) }}</div>
                                    @if($pago->cambio > 0)
                                        <div class="text-emerald-600 dark:text-emerald-400">
                                            Cambio: L. {{ number_format($pago->cambio, 2) }}
                                        </div>
    ...
    <script>
        function confirmReportePagos() {
            ...
            Swal.fire({
                ...
                text: 'Seleccione la tutoria para ver todos los ingresos:',
                input: 'select',
                ...
                if (result.isConfirmed && result.value) {
                    @this.downloadReportePagosTutoria(result.value);
                }
            });
        }
    </script>
    @include('livewire.pagos-tutoria.create')
    @include('livewire.pagos-tutoria.delete-confirmation')
    @include('livewire.pagos-tutoria.error-modal')
</div>