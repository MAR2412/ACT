                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    @if(!$matricula->trashed())
                                        ...
                                        @endcan
                                        @if($matricula->saldo_pendiente == 0 && !$matricula->trashed())
                                            <button wire:click="generarReciboFiniquito({{ $matricula->id }})"
                                                    class="text-emerald-600 hover:text-emerald-900 dark:text-emerald-400 dark:hover:text-emerald-300"
                                                    title="Generar Recibo de Finiquito (Módulo Pagado)">
                                ...
                                <div class="flex space-x-2">
                                    @if(!$matricula->trashed())
                                        ...
                                        @if($matricula->saldo_pendiente == 0 && !$matricula->trashed())
                                            <button wire:click="generarReciboFiniquito({{ $matricula->id }})"
                                                    class="text-emerald-600 hover:text-emerald-900 dark:text-emerald-400 dark:hover:text-emerald-300"
                                                    title="Generar Recibo de Finiquito">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                                </svg>
                                            </button>
