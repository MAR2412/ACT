<div>
        ...
        <div class="bg-white dark:bg-white/5 overflow-hidden shadow sm:rounded-lg p-4 sm:p-6">
            ...
            <div class="mb-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                    <h2 class="text-xl font-semibold text-stone-800 dark:text-stone-200">
                        {{ __('Administración de Tutorías') }}
                    </h2>

                    <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                        <div class="relative w-full sm:w-auto">
                            <x-input wire:model.live="search" type="text" placeholder="Buscar tutorías, materias, sedes..."
                        ...
                        @can('oferta-educativa.tutorias.crear')
            ...
            <x-table
                ...
                :columns="[
                    ...
                    ['key' => 'nombre', 'label' => 'Tutoría', 'sortable' => true],
                    ...
                    @forelse($tutorias as $tutoria)
                        <tr class="hover:bg-stone-50 dark:hover:bg-stone-700 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                ...
                                @if($tutoria->descripcion)
                                    <div class="text-sm text-stone-500 dark:text-stone-400 mt-1">
                                        {{ Str::limit($tutoria->descripcion, 50) }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                <div class="text-sm">
                                    <span class="font-medium">Sede:</span> {{ $tutoria->sede->nombre }}<br>
                                    <span class="font-medium">Ubicación:</span> {{ $tutoria->sede->municipio }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                <div class="text-sm">
                                    <span class="font-medium">Modalidad:</span> {{ $tutoria->modalidad->nombre }}<br>
                                    <span class="font-medium">Sección:</span> {{ $tutoria->seccion->nombre }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                <div class="font-medium">{{ $tutoria->precio_hora_formateado }}/hora</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <button wire:click="toggleEstado({{ $tutoria->id }})"
                                        class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 {{ $tutoria->estado ? 'bg-emerald-600' : 'bg-gray-200 dark:bg-gray-700' }}">
                                        <span class="sr-only">Cambiar estado</span>
                                        <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {{ $tutoria->estado ? 'translate-x-6' : 'translate-x-1' }}"></span>
                                    </button>
                                    <span class="ml-2 text-sm text-stone-600 dark:text-stone-400">
                                        {{ $tutoria->estado ? 'Activo' : 'Inactivo' }}
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    @if($tutoria->trashed())
                                        @can('oferta-educativa.tutorias.restaurar')
                                            <button wire:click="restore({{ $tutoria->id }})"
                                        ...
                                        @endcan
                                    @else
                                        @can('oferta-educativa.tutorias.editar')
                                            <button wire:click="edit({{ $tutoria->id }})"
                                        ...
                                        @endcan
                                        @can('oferta-educativa.tutorias.eliminar')
                                            <button wire:click="confirmDelete({{ $tutoria->id }})"
                ...
                <x-slot name="mobile">
                    @forelse($tutorias as $tutoria)
                        <div class="bg-white dark:bg-stone-800 p-4 rounded-lg shadow-sm border border-stone-200 dark:border-stone-700 mb-3">
                            ...
                            <h3 class="font-semibold text-stone-900 dark:text-stone-200 text-lg mb-1">{{ $tutoria->nombre }}</h3>
                            @if($tutoria->materia)
                                <div class="inline-block bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 px-2 py-0.5 rounded-full text-xs mb-2">
                                    {{ $tutoria->materia }}
                                </div>
                            @endif
                            @if($tutoria->descripcion)
                            ...
                            <div class="grid grid-cols-2 gap-2 text-sm mb-2">
                                <div class="text-stone-600 dark:text-stone-400">
                                    <span class="font-medium">Sede:</span><br>
                                    {{ $tutoria->sede->nombre }}<br>
                                    <span class="text-xs">{{ $tutoria->sede->municipio }}</span>
                                </div>
                                <div class="text-stone-600 dark:text-stone-400">
                                    <span class="font-medium">Modalidad:</span><br>
                                    {{ $tutoria->modalidad->nombre }}<br>
                                    <span class="font-medium">Sección:</span> {{ $tutoria->seccion->nombre }}
                                </div>
                            </div>
                            <div class="text-sm mb-3">
                                <div class="text-stone-600 dark:text-stone-400">
                                    ...
                                    <span class="font-semibold">{{ $tutoria->precio_hora_formateado }}/hora</span>
                                </div>
                            </div>
    ...
    <!-- Incluir modales -->
    @include('livewire.tutoria.create')
    @include('livewire.tutoria.delete-confirmation')
    @include('livewire.tutoria.error-modal')
</div>