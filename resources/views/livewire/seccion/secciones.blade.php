<div>
        ...
        <div class="bg-white dark:bg-white/5 overflow-hidden shadow sm:rounded-lg p-4 sm:p-6">
            ...
            <div class="mb-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                    <h2 class="text-xl font-semibold text-stone-800 dark:text-stone-200">
                        {{ __('Administración de Secciones') }}
                    </h2>

                    <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                        <div class="relative w-full sm:w-auto">
                            <x-input wire:model.live="search" type="text" placeholder="Buscar por nombre o día..."
            ...
            <x-table
                ...
                :columns="[
                    ...
                    ['key' => 'dia', 'label' => 'Días', 'sortable' => true],
                    ['key' => 'horario', 'label' => 'Horario'],
                    ['key' => 'estado', 'label' => 'Estado'],
                    ['key' => 'actions', 'label' => 'Acciones', 'class' => 'text-right'],
                ]"
                empty-message="No se encontraron secciones"
                class="mt-6"
            >
                <x-slot name="desktop">
                    @forelse($secciones as $seccion)
                        <tr class="hover:bg-stone-50 dark:hover:bg-stone-700 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">{{ $seccion->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">{{ $seccion->nombre }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                {{ $seccion->dia }}{{ $seccion->diaF ? ' a ' . $seccion->diaF : '' }}
                            </td>
                            <td class="px-6 py-4 text-stone-900 dark:text-stone-300">
                                {{ \Carbon\Carbon::parse($seccion->HoraInicio)->format('h:i A') }} - 
                                {{ \Carbon\Carbon::parse($seccion->HoraFin)->format('h:i A') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <button wire:click="toggleEstado({{ $seccion->id }})"
                                        class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 {{ $seccion->estado ? 'bg-emerald-600' : 'bg-gray-200 dark:bg-gray-700' }}">
                                        <span class="sr-only">Cambiar estado</span>
                                        <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {{ $seccion->estado ? 'translate-x-6' : 'translate-x-1' }}"></span>
                                    </button>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    @if($seccion->trashed())
                                        @can('secciones.secciones.restaurar')
                                            <button wire:click="restore({{ $seccion->id }})"
                                        ...
                                        @endcan
                                    @else
                                        @can('secciones.secciones.editar')
                                        ...
                                        @endcan
                                        @can('secciones.secciones.eliminar')
                ...
                <x-slot name="mobile">
                    @forelse($secciones as $seccion)
                        <div class="bg-white dark:bg-stone-800 p-4 rounded-lg shadow-sm border border-stone-200 dark:border-stone-700 mb-3">
                            ...
                            <h3 class="font-semibold text-stone-900 dark:text-stone-200 text-lg mb-1">{{ $seccion->nombre }}</h3>
                            <div class="grid grid-cols-2 gap-2 text-sm mb-2">
                                <div class="text-stone-600 dark:text-stone-400">
                                    <span class="font-medium">Día:</span> {{ $seccion->dia }}
                                </div>
                                <div class="text-stone-600 dark:text-stone-400">
    ...
    <!-- Incluir modales -->
    @include('livewire.seccion.create')
    @include('livewire.seccion.delete-confirmation')
    @include('livewire.seccion.error-modal')
</div>