        ...
        <div class="bg-white dark:bg-white/5 overflow-hidden shadow sm:rounded-lg p-4 sm:p-6">
            ...
            <div class="mb-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                    <h2 class="text-xl font-semibold text-stone-800 dark:text-stone-200">
                        {{ __('Administración de Módulos') }}
                    </h2>

                    <div class="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-2">
                        <div class="relative w-full sm:w-auto">
                            <x-input wire:model.live="search" type="text" placeholder="Buscar módulos, sedes, modalidades..."
                        ...
                        @can('oferta-educativa.modulos.crear')
            ...
            <x-table
                ...
                :columns="[
                    ...
                    ['key' => 'nombre', 'label' => 'Módulo', 'sortable' => true],
                ...
                <x-slot name="desktop">
                    @forelse($modulos as $modulo)
                        <tr class="hover:bg-stone-50 dark:hover:bg-stone-700 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">{{ $modulo->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                ...
                                @if($modulo->descripcion)
                                    <div class="text-sm text-stone-500 dark:text-stone-400 mt-1">
                                        {{ Str::limit($modulo->descripcion, 50) }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                <div class="text-sm">
                                    <span class="font-medium">Sede:</span> {{ $modulo->sede->nombre }}<br>
                                    <span class="font-medium">Ubicación:</span> {{ $modulo->sede->municipio }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                <div class="text-sm">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-stone-900 dark:text-stone-300">
                                <div class="font-medium">{{ $modulo->precio_mensual_formateado }}/mes</div>
                                <div class="text-sm text-stone-500 dark:text-stone-400">
                                    Total: {{ $modulo->precio_total_formateado }}
                                </div>
                                @if($modulo->fecha_inicio)
                                    <div class="text-xs text-stone-500 dark:text-stone-400 mt-1">
                                        Inicia: {{ $modulo->fecha_inicio->format('d/m/Y') }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <button wire:click="toggleEstado({{ $modulo->id }})"
                                        class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 {{ $modulo->estado ? 'bg-emerald-600' : 'bg-gray-200 dark:bg-gray-700' }}">
                                        <span class="sr-only">Cambiar estado</span>
                                        <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {{ $modulo->estado ? 'translate-x-6' : 'translate-x-1' }}"></span>
                                    </button>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    @if($modulo->trashed())
                                        @can('oferta-educativa.modulos.restaurar')
                                            <button wire:click="restore({{ $modulo->id }})"
                                        ...
                                        @endcan
                                    @else
                                        @can('oferta-educativa.modulos.editar')
                                            <button wire:click="edit({{ $modulo->id }})"
                ...
                <x-slot name="mobile">
                    @forelse($modulos as $modulo)
                        <div class="bg-white dark:bg-stone-800 p-4 rounded-lg shadow-sm border border-stone-200 dark:border-stone-700 mb-3">
                            ...
                            <h3 class="font-semibold text-stone-900 dark:text-stone-200 text-lg mb-1">{{ $modulo->nombre }}</h3>
                            ...
                            @if($modulo->descripcion)
                                <p class="text-sm text-stone-600 dark:text-stone-400 mb-2">
                                    {{ Str::limit($modulo->descripcion, 80) }}
                                </p>
                            @endif
                            <div class="grid grid-cols-2 gap-2 text-sm mb-2">
                                <div class="text-stone-600 dark:text-stone-400">
                                    <span class="font-medium">Sede:</span><br>
                                    {{ $modulo->sede->nombre }}<br>
                                    <span class="text-xs">{{ $modulo->sede->municipio }}</span>
                                </div>
                                <div class="text-stone-600 dark:text-stone-400">
                            ...
                            <div class="grid grid-cols-2 gap-2 text-sm mb-3">
                                ...
                                <div class="text-stone-600 dark:text-stone-400">
                                    ...
                                    <span class="font-semibold">{{ $modulo->precio_mensual_formateado }}/mes</span><br>
                                    <span class="text-xs">Total: {{ $modulo->precio_total_formateado }}</span>
                                </div>
                            </div>
                            @if($modulo->moduloRequerido)
                                <div class="text-sm text-stone-600 dark:text-stone-400 mb-3">
                                    <span class="font-medium">Requiere:</span> {{ $modulo->moduloRequerido->nombre }}
                                </div>
                            @endif
                            @if($modulo->fecha_inicio)
                                <div class="text-sm text-stone-600 dark:text-stone-400 mb-3">
                                    <span class="font-medium">Inicia:</span> {{ $modulo->fecha_inicio->format('d/m/Y') }}
                                </div>
                            @endif
